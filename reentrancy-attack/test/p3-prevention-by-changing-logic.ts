...
describe("Attack", function () {
  ...
  async function fixture() {
    ...
    const myEtherWallet_ = await ethers.getContractFactory("MyEtherWallet3");
    const myEtherWallet = await myEtherWallet_.deploy();

    //bad user deploy the attacker contract
    const attacker_ = await ethers.getContractFactory("Attacker3");
  ...
  describe("Prevention by changing logic", function () {
    it("Should not transfer more", async function () {
